...
/* Work sbuffer for filesystem */
/* Буфер для работы файловой системы */

// Интерфейс к чтению-записи карты

unsigned char read_block(unsigned char* buf, unsigned long adr, unsigned int length, unsigned int offset);
unsigned char write_block(unsigned char* buf,unsigned long adr);


extern unsigned char* getsbufptr();
...
#define FS_DIRENTRY  (sbuf + 480)

/* Functions. Returns not 0 if error occured. Destroy sbuf variable*/
/* Функцяии. Возращают не 0, если ошибка. Все функции портят sbuf */
...
BYTE fs_delete();                                /* Удалить файл или папку, имя в sbuf / Delete file or folder, name in sbuf */
BYTE fs_open0(BYTE what);                        /* Открыть/создать файл или папку, имя в sbuf. Open/create file or foder, name in sbuf */
BYTE fs_move0();                                 /* Переместить файл/папку. Move file or folder, source file must been opened, destination name in sbuf */
...
BYTE fs_write(BYTE* ptr, WORD len);        /* Записать в файл, увеличивает размер файла. / Записать в файл, enlarge file size. */
...
void fs_select(char fn);                         /* Переключится на другой файл / Switch to the second file */
#endif
...
/* Функции ниже позволяют использовать для записи буфер (sbuf).
*  The functions below can be used to write the sbuffer (sbuf).
*
*  wtotal = размер данных для записи;
*  while(wtotal) {
*    fs_write_start(&wtotal);
*    Копируем fs_file_wlen байт в fs_file_wsbuf
...
#define fs_file_wsbuf   (sbuf + fs_file_woff)
