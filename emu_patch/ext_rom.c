...
// Диск 4 (E) - Mount-диск
strcpy(drive_filename[4],"EXRTOOLS.DSK");  
...
void emu_read_image_128(void) {
  ...
  if ((e_substitute_system_track == 1) && (e_trk<2) && (e_drv == 0)) strcat(tmp_path,sysfiles[substitute_number]); // для режима подстановки образа системы
  else  { 
...
void emu_api_cmd(void) {
  ...
  switch (in_buffer[0]) {
	...
	if (e_trk == 0) e_substitute_system_track=0;
	  ...
	  sysfiles[2][6]=e_trk;    // впиывваем # в SYSTEMn.BIN
