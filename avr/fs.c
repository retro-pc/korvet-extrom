...
static unsigned char sbuf[512];         // буфер сектора SD-карты
...
File fcb[6];     // описатели всех доступных файлов
File* fs_file;   // указатель на текущий открытый файл
int fnum;        // номер описателя текущего обрабатываемого файла      
...
unsigned char sd_read(unsigned char* buf,DWORD sector, WORD offsetInSector, WORD length) {
 return read_block(buf,sector*512,length,offsetInSector);
} 

unsigned char sd_write512(unsigned char* buf, DWORD sector) {
 return write_block(buf,sector*512);
} 
...
static BYTE sd_readBuf(DWORD sector) {
  return sd_read(sbuf, sector, 0, 512);
}
...
static BYTE sd_writeBuf(DWORD sector) {
  return sd_write512(sbuf, sector);
}
...
BYTE fs_init() {
  ...
  for(i=0;i<5;i++) fcb[i].opened = OPENED_NONE;
  fs_type = FS_ERROR;
  ...
  while(1) {                 
    ...
    if(LD_WORD(sbuf + 510) == 0xAA55) {
      if(LD_WORD(sbuf + BS_FilSysType  ) == 0x4146) break;
      if(LD_WORD(sbuf + BS_FilSysType32) == 0x4146) break;
      /* Возможно это MBR */       
      if(bsect == 0 && sbuf[MBR_Table+4]) { 
        bsect = LD_DWORD(sbuf + (MBR_Table + 8));
        if(bsect != 0) continue;
      }      
    }   
abort_noFS:
  ...
  fsize = LD_WORD(sbuf + BPB_FATSz16);
  if(fsize == 0) fsize = LD_DWORD(sbuf + BPB_FATSz32);
  
  /* Размер файловой системы в секторах */
  tsect = LD_WORD( sbuf + BPB_TotSec16);  
  if(tsect == 0) tsect = LD_DWORD(sbuf + BPB_TotSec32);
  
  /* Размер корневого каталога (должно быть кратно 16 и для FAT32 должно быть рано нулю) */
  fs_n_rootdir = LD_WORD(sbuf + BPB_RootEntCnt);
  
  /* Адреса таблиц FAT в секторах */
  fs_fatbase  = bsect + LD_WORD(sbuf + BPB_RsvdSecCnt);
  fs_fatbase2 = 0;
  if(sbuf[BPB_NumFATs] >= 2) fs_fatbase2 = fs_fatbase + fsize;

  /* Кол-во секторов на кластер */
  fs_csize = sbuf[BPB_SecPerClus];

  /* Кол-во кластеров */
  fsize *= sbuf[BPB_NumFATs];
  fs_n_fatent = (tsect - LD_WORD(sbuf + BPB_RsvdSecCnt) - fsize - fs_n_rootdir / 16) / fs_csize + 2;
  ...
  /* FAT 32 */
  fs_dirbase = LD_DWORD(sbuf + BPB_RootClus);
  
  /* Сбрасываем счетчик свободного места */
  if(LD_WORD(sbuf + BPB_RsvdSecCnt)>0) {
    bsect++;
    if(sd_readBuf(bsect)) return 1;
    if(LD_DWORD(sbuf) == 0x41615252 && LD_DWORD(sbuf + 0x1E4) == 0x61417272 && LD_DWORD(sbuf + 0x1FC) == 0xAA550000) {
      LD_DWORD(sbuf + 0x1E8) = 0xFFFFFFFF;
      LD_DWORD(sbuf + 0x1EC) = 0xFFFFFFFF;
...
*  Функция не портит sbuf (функции, в которых этого не написано, портят)   *
**************************************************************************/
...
*  приведено                                                              *
*                                                                         *
*  Функция не портит sbuf[0..MAX_FILENAME-1]                               *
***************************************************************************/
...
static BYTE fs_allocCluster(DIS(BYTE freeSpace)) {
  ...
  while(1) {
    /* Сектор и смещение */ 
    s = fs_tmp / 256, i = (BYTE)fs_tmp, a = (BYTE*)((WORD*)sbuf + i);    
    if(fs_type != FS_FAT16) s = fs_tmp / 128, i |= 128, a = (BYTE*)((DWORD*)sbuf - 128 + i); 
...
static BYTE fs_setNextCluster(DWORD cluster) {
  ...
  if(fs_type == FS_FAT16) {                                                                                    
    a = (WORD*)sbuf + (BYTE)cluster;
    prev = LD_WORD(a);
    LD_WORD(a) = (WORD)fs_tmp;
  } else {
    a = (DWORD*)sbuf + (BYTE)cluster % 128;
    prev = LD_DWORD(a);
    LD_DWORD(a) = fs_tmp;
  }
...
static BYTE* fs_allocEntry() {
  ...
  while(1) {
    ...
    if(FS_DIRENTRY[0] == 0xE5 || FS_DIRENTRY[0] == 0) { /* Может быть еще 0x05 */
      ...
      /* Найденный описатель */
      return sbuf + (fs_file->entry_index % 16) * 32;
    }
  }               
...
*                                                                         *
*  Имя файла в sbuf. Оно не должно превышать FS_MAXFILE симолов включая 0  *
*                                                                         *
...
static  BYTE* fs_open0_name( BYTE *p); /* forward */

#define FS_DIRFIND      (sbuf + 469)           /* 11 байт использующиеся внутри функции fs_open0 */
...
BYTE fs_open0(BYTE what) {
  ...
  if(sbuf[0] == 0) {	
    ...
    while(1) {
      ...
      if(path == ( BYTE*)1) goto abort_noPath;      	
      /* Ищем имя в папке */      
      fs_file->entry_able = 0;
      while(1) {              
...
//static BYTE exists(const PROGMEM BYTE* str, BYTE c) {
//  while(*str)   
//    if(*str++ == c) 
//      return c;
//  return 0;
...
static  BYTE * fs_open0_name( BYTE *p) {
  BYTE c, ni, i;
...
static BYTE fs_open0_create(BYTE dir) {
  ...
  /* Очищаем fs_tmp и за одно sbuf */
  fs_eraseCluster(1); 
  
  /* Создаем пустую папку */
  memset(sbuf, ' ', 11); sbuf[0] = '.'; sbuf[11] = 0x10;
  fs_setEntryCluster(sbuf, allocatedCluster);         
  
  memset(sbuf+32, ' ', 11); sbuf[32] = '.'; sbuf[33] = '.'; sbuf[32+11] = 0x10;
  if(fs_notrootdir) fs_setEntryCluster(sbuf + 32, fs_file->entry_start_cluster); /* Сейчас в fs_file->size==0 значит корневой каталог */
...
static char fs_saveFileLength() {
  ...
  entry = sbuf + (fs_file->entry_index % 16) * 32;
...
BYTE fs_move0() {
  ...
  if(entry[DIR_Attr] & AM_DIR) {
    ...
    fs_setEntryCluster(sbuf+32, fs_parent_dir_cluster); /* Первый кластер папки предка.*/
...
*  Имя файла должно содержаться в sbuf и не превышать FS_MAXFILE симолов   *
*  включая терминатор                                                     *
**************************************************************************/
...
BYTE fs_write( BYTE* ptr, WORD len) {
  ...
  do {
    ...
    memcpy(fs_file_wsbuf, ptr, fs_file_wlen);
    ptr += fs_file_wlen;
...
/**************************************************************************
*  Переключить файлы - выбираем текущим один из файлов 0-4                 *
**************************************************************************/

void fs_select(char fn) {
...
extern unsigned char* getsbufptr() { return sbuf;}
